              ...
              <div class="box-body">
                <table id="example1" class="table table-bordered">
                  ...
                  <tbody>
                    ...
                    $sql = "SELECT *, SUM(amount) as total_amount FROM bonuses";
                    $query = $conn->query($sql);
                    $brow = $query->fetch_assoc();
                    $bonuses_total = $brow['total_amount'];
                    ...
                    $sql = "SELECT *, SUM(num_hr) AS total_hr, 
                            (SELECT SUM(overtime.hours) FROM overtime WHERE overtime.employee_id = attendance.employee_id AND overtime.date_overtime BETWEEN '$from' AND '$to') AS total_overtime, 
                    ...
                    while ($row = $query->fetch_assoc()) {
                      ...
                      $net = $gross - $total_deduction + $bonuses_total;
                      echo "
                        <tr>
                          <td>" . $row['lastname'] . ", " . $row['firstname'] . "</td>
                          <td>" . $row['employee_id'] . "</td>
                          <td>" . number_format($gross, 2) . "</td>
                          <td>" . number_format($bonuses_total, 2) . "</td>
  ...
  <script>
    $(function() {
      ...
      $('#payroll').click(function(e) {
        e.preventDefault();
        $('#payForm').attr('action', 'nomina_sueldo.php');
        $('#payForm').submit();
      });
